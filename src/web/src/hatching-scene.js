...
function randomEdgePoint(rng, w, h, margin = 60) {
  ...
  switch (edge) {
    case 0: return { x: rng() * w, y: -margin, edge: 0 }
    case 1: return { x: w + margin, y: rng() * h, edge: 1 }
    case 2: return { x: rng() * w, y: h + margin, edge: 2 }
    default: return { x: -margin, y: rng() * h, edge: 3 }
  }
}
...
function oppositeEdgePoint(rng, entryEdge, w, h, margin = 80) {
  ...
  switch (entryEdge) {
    case 0: return { x: margin + rng() * (w - margin * 2), y: h + margin } // top→bottom
    case 1: return { x: -margin, y: margin + rng() * (h - margin * 2) }     // right→left
    case 2: return { x: margin + rng() * (w - margin * 2), y: -margin }     // bottom→top
    default: return { x: w + margin, y: margin + rng() * (h - margin * 2) } // left→right
  }
}
...
function exitPoint(x, y, w, h, margin = 80) {
  ...
  let best = dists[0], bestD = Infinity
...
class Ant {
  ...
  update(dt) {
    ...
    if (this.state === 'crossing') {
      // Off-screen → done
      if (
        this.x < -100 || this.x > this.w + 100 ||
        this.y < -100 || this.y > this.h + 100
      ) {
        this.state = 'gone'
      }
