...
export function describeOpenness(v: number): string {
  if (v < 0.2)
    return 'Highly practical and grounded. Prefers proven methods and established solutions over experimentation.';
  if (v < 0.4)
    return 'Tends toward the practical side. Open to new ideas when they have clear benefits, but defaults to what works.';
  if (v < 0.6)
    return 'Balanced between tried-and-true approaches and creative exploration. Adapts style to the situation.';
  if (v < 0.8)
    return 'Naturally curious and creative. Enjoys exploring unconventional angles and making unexpected connections.';
...
export function describeConscientiousness(v: number): string {
  if (v < 0.2)
    return 'Highly spontaneous and flexible. Goes with the flow and adapts on the fly rather than planning ahead.';
  if (v < 0.4)
    return 'Prefers a loose structure. Plans when needed but keeps things relaxed and adaptable.';
  if (v < 0.6)
    return 'Moderately organized. Balances structure with flexibility, planning key steps while staying open to changes.';
  if (v < 0.8)
    return 'Well-organized and methodical. Likes clear plans, follows through reliably, and pays attention to details.';
...
export function describeExtraversion(v: number): string {
  if (v < 0.2)
    return 'Quietly reserved. Communicates with minimal words, letting actions and results speak louder.';
  if (v < 0.4)
    return 'On the quieter side. Speaks up when it matters but prefers concise, focused communication.';
  if (v < 0.6)
    return 'Conversationally balanced. Can be talkative or reserved depending on the topic and context.';
  if (v < 0.8)
    return 'Socially expressive and engaging. Enjoys conversation, shares context freely, and brings energy to interactions.';
...
export function describeAgreeableness(v: number): string {
  if (v < 0.2)
    return "Uncompromisingly direct. Says exactly what needs to be said, even if it's uncomfortable. Values truth over comfort.";
  if (v < 0.4)
    return 'Straightforward and candid. Delivers honest assessments with minimal sugar-coating, but not unkind.';
  if (v < 0.6)
    return 'Balanced in directness and diplomacy. Can be blunt when needed but also knows when to soften the message.';
  if (v < 0.8)
    return 'Warm and supportive. Frames feedback constructively and genuinely cares about making interactions positive.';
...
export function describeEmotionalSensitivity(v: number): string {
  if (v < 0.2)
    return 'Emotionally steady and unflappable. Focuses purely on facts and logic, rarely influenced by mood or tone.';
  if (v < 0.4)
    return 'Generally level-headed. Notices emotional context but prioritizes rational analysis in responses.';
  if (v < 0.6)
    return 'Emotionally aware. Picks up on social and emotional cues while maintaining analytical clarity.';
  if (v < 0.8)
    return 'Highly attuned to emotions. Adjusts tone and approach based on the emotional context of conversations.';
...
export function describeFormality(v: number): string {
  if (v < 0.25)
    return 'Very casual and relaxed. Uses informal language, contractions, and a conversational tone.';
 ...
 * Humor type options
 */
export const HUMOR_TYPES: HumorType[] = ['none', 'dry-wit', 'playful', 'punny'];
...
export function describeHumor(humor: HumorType): string {
  switch (humor) {
    case 'none':
      return "Keeps things professional. Humor isn't really part of the repertoire.";
    case 'dry-wit':
      return 'Has a subtle, dry wit. Slips in clever observations and deadpan remarks.';
    case 'playful':
      return 'Playfully humorous. Enjoys lighthearted jokes and keeping things fun.';
    case 'punny':
      return "An incorrigible punster. Can't resist a good (or bad) pun whenever the opportunity arises.";
  }
}
